...
namespace LoRaWAN.Data.Repositories
{
    public abstract class EfRepository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        public EfRepository(LoRaWANDBContext context)
        {
            _context = context;
            _entities = Entities;
        }
        ...
        public virtual List<TEntity> GetList(Expression<Func<TEntity, bool>> criteria, DataRequest request)
        {
            return Entities
                    .AsQueryable()
                    .CriteriaOrDefault(criteria)
                    .SortOrDefault(request)
                    .ToList();
        }
